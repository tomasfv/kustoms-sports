...
export const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case types.GET_PRODUCTINFO:
      const images = action.payload.products.map((e) => {
        return e.image[0]
      })
    ...
    case types.FILTER:
      ...
      if (
        filter.brand === 'all' &&
        filter.clotheType === 'all' &&
        filter.collection === 'all' &&
        filter.gender === 'all' &&
        filter.color === 'all' /* && filter.size==='all' */ &&
        filter.sport === 'all'
      ) {
        return {
          ...state,
        ...
        if (filter.clotheType !== 'all') {
          if (finalShow.length) {
            finalShow = finalShow.filter(
              (e) => e.clotheType === filter.clotheType,
            )
          } else {
            finalShow = toFilter.filter(
              (e) => e.clotheType === filter.clotheType,
            )
          }
          checker()
        ...
        if (filter.collection !== 'all') {
          if (finalShow.length) {
            finalShow = finalShow.filter(
              (e) => e.collection === filter.collection,
            )
          } else {
            finalShow = toFilter.filter(
              (e) => e.collection === filter.collection,
    ...
    case types.DASH_FILTER:
      ...
      let dashfinalShow = []
      if (
        dashfilter.brand === 'all' &&
        dashfilter.clotheType === 'all' &&
        dashfilter.collection === 'all' &&
        dashfilter.gender === 'all' &&
        dashfilter.color === 'all' /* && filter.size==='all' */ &&
        dashfilter.sport === 'all'
      ) {
        return {
          ...state,
        ...
        if (dashfilter.brand !== 'all') {
          if (dashfinalShow.length) {
            dashfinalShow = dashfinalShow.filter(
              (e) => e.brand === dashfilter.brand,
            )
          } else {
            dashfinalShow = dashtoFilter.filter(
              (e) => e.brand === dashfilter.brand,
            )
          }
          checker()
        ...
        if (dashfilter.clotheType !== 'all') {
          if (dashfinalShow.length) {
            dashfinalShow = dashfinalShow.filter(
              (e) => e.clotheType === dashfilter.clotheType,
            )
          } else {
            dashfinalShow = dashtoFilter.filter(
              (e) => e.clotheType === dashfilter.clotheType,
        ...
        if (dashfilter.collection !== 'all') {
          if (dashfinalShow.length) {
            dashfinalShow = dashfinalShow.filter(
              (e) => e.collection === dashfilter.collection,
            )
          } else {
            dashfinalShow = dashtoFilter.filter(
              (e) => e.collection === dashfilter.collection,
