...
const Carrito = () => {
  ...
  if (dataBuy.length > 0) {
    ...
    final = descuento.reduce((acc, elemento) => {
      return acc + elemento
    })
    descuento2 = dataBuy.map((e) => Math.round(e.price * e.promotion))
    descuento2 = descuento2.reduce((acc, e) => {
      return acc + e
    })
    precios = dataBuy.map((e) => Math.round(e.price))
    precios = precios.reduce((acc, e) => {
      return acc + e
    })
  }
        ...
        <div className=" flex flex-row gap-[300px] mb-[100px]">
          <div>
            <div className="flex flex-col ">
              {data.totalamount === undefined || data.totalamount === 0 ? (
                <div className="flex flex-col ml-[200px] items-start justify-items-start mt-[10px] dark:text-main-light">
                  <p>Una vez que realices un pedido,aparecerá acá.</p>
                  <p>
                    Podrías comenzar viendo nuestra nueva colección de{' '}
                    <Link
                      to="/categories/collection/Qatar"
                            ...
                            <>
                              <div>
                                <div className="flex flex-row">
                                  Precio por unidad:{' '}
                                  <p className="ml-10 line-through text-red-dark">
                                    ${e.price}
                                  </p>
                                </div>
                                <p className="flex flex-row">
                                  Precio con descuento:
                                  <p className="text-verde-dark font-bold ml-2">
                                    ${Math.round(e.price * (1 - e.promotion))}
                                  </p>
                                </p>
                              </div>
                            </>
          ...
          <div className="flex flex-col w-[400px] h-[400px] bg-gris-light border-2 mt-[50px]">
            ...
            <div className="flex flex-row mt-[20px]">
              <div className="flex font-bold text-[25px] ml-[15px]">
                Subtotal:
                {/* <div className=" flex ml-[154px] mr-[200px]">${data.totalamount} */}
                <div className=" flex ml-[154px] mr-[200px]">${precios}</div>
              </div>
            </div>
            <div className="flex flex-row mt-[20px]">
              <div className="flex font-bold text-[25px] ml-[15px]">
                ...
                <div className=" flex ml-[124px] mr-[200px]">${descuento2}</div>
              </div>
            </div>
            <div className="flex flex-row mt-[80px]">
              <div className="flex font-bold text-[25px] ml-[15px]">
                Total:
                <div className=" flex ml-[200px] mr-[200px]">${final}</div>
              </div>
            </div>

            {/* {console.log("data",data)} */}
            {data.totalamount === undefined || final === 0 ? (
              <p className="border text-main-light bg-main-dark w-[350px] ml-[25px] text-[20px]  justify-center items-center mt-[20px] p-2">
                Tu carrito está vacío
              </p>
            ) : (
              <a
                href="/payment"
                className="border text-[white] bg-verde-light w-[350px] ml-[25px] text-[20px]  justify-center items-center mt-[20px] p-2"
              >
                COMPRAR
              </a>
            )}
