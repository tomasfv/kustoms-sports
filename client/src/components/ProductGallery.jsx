...
const ProductGallery = ({ productos }) => {
  ...
  const handleRightClick = () => {
    page < amountPages.length && setPage(page + 1)
  }
  ...
  const Pagination = () => (
    <div className="mt-6 flex flex-row mx-auto justify-center gap-6 items-center">
      <button
        ...
        className="text-main-dark dark:text-main-light cursor-pointer hover:bg-opacity-10 hover:bg-main-dark dark:hover:bg-main-light dark:hover:text-main-dark p-2 rounded-full"
      >
        <MdChevronLeft />
      </button>
      {amountPages &&
        amountPages.map((e, index) => {
          e = e + 1
          return (
            <button
              key={index}
              onClick={() => handleClick(e)}
              className={`text-main-dark dark:text-main-light ${
                e === page
                  ? ' text-main-dark bg-verde-light dark:bg-verde-dark'
                  : 'hover:bg-opacity-10 hover:bg-main-dark dark:hover:bg-main-light dark:hover:text-main-dark'
              } p-2 rounded-full `}
            >
              {e}
            </button>
      ...
      <button
        ...
        disabled={page === amountPages.length ? true : false}
        className="text-main-dark dark:text-main-light cursor-pointer hover:bg-opacity-10 hover:bg-main-dark dark:hover:bg-main-light dark:hover:text-main-dark p-2 rounded-full"
      >
        <MdChevronRight
          onClick={handleRightClick}
          className="text-main-dark dark:text-main-light cursor-pointer "
        />
      </button>
    </div>
  )
      ...
      <section className="w-10/12 my-5 mx-auto flex flex-row flex-wrap gap-6 justify-center">
        {onDisplay[0] === 'vacio' ? (
          <section className="w-full shadow-lg border rounded-md py-10 text-main-dark dark:text-main-light bg-main-light dark:bg-main-dark border-verde-light dark:border-verde-dark">
            La busqueda no produjo ningun resultado, intente cambiando algunos
            parametros
          </section>
        ) : (
