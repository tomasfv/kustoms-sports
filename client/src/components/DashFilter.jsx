...
import { dashfilterProducts } from '../redux/actions'
import { MdExpandMore } from 'react-icons/md'

const DashFilter = ({ data }) => {
  ...
  useEffect(() => {
    dispatch(dashfilterProducts(filterProduct))
  }, [filterProduct])
  return (
          ...
          <div className="relative">
            <select
              ...
              className={` z-20 md:w-52 w-fit h-14 uppercase text-center
                    ...
                    bg-main-light  dark:bg-main-dark  border-b-[5px] `}
              onChange={(event) => handleSelect(e, event.target)}
            >
              <option className={`uppercase `} defaultValue value={'all'}>
                {e}
              </option>
              {filters[e]?.map((j, index) => {
                return (
                  <option key={j + index + e} value={j}>
                    {j}
                  </option>
                )
              })}
            ...
            <p
              className={`z-10 absolute uppercase flex items-center ${
                filterProduct[translate(e)] === { e }
                  ? 'top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4'
                  : 'text-xs'
              } cursor-default transition-[font-size] duration-500 `}
            >
              {e}{' '}
              <MdExpandMore
                className={` uppercase ${
                  filterProduct[translate(e)] === 'all' ? 'block' : 'hidden'
                } cursor-default transition-all duration-700 `}
              />
            </p>
          </div>
        )
