...
router.get('/', async (req, res) => {
  ...
  try {
    if (sport) {
      const productsport = await Products.findAll({
        where: { sport: sport, available: true },
      })
      ...
      productsport.forEach((element) => {
        ...
        for (let i = 0; i < aux.length; i++) {
          if (element.name === aux[i].name && element.gender === aux[i].gender)
            flag = false
        }
      ...
      const productsname = await Products.findAll({
        where: { name: { [Op.substring]: name }, available: true },
      })
      let aux = []
      ...
      const productscollection = await Products.findAll({
        where: { collection: collection, available: true },
      })
      let aux = []
      // aux.push(productsport[0])
      ...
      const productsgender = await Products.findAll({
        where: { gender: gender, available: true },
      })
      let aux = []
      // aux.push(productsport[0])
      ...
      const productsbrand = await Products.findAll({
        where: { brand: brand, available: true },
      })
      let aux = []
      // aux.push(productsport[0])
